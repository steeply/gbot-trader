...
TRADING_PRICE_RANGE_PERCENT         | Процент для автоматического создания диапазон цен разрешенных для торгов.<br>Формат `number` или `bottom/top`<br>Если не указано, нет ограничений<br>(FAQ п.51)  | number or string | -
...
PRICE_TRADING_TYPE_CHANGE           | Цена при которой изменится тип торговли<br>Формат `number` или `low/high`<br>(FAQ п.58) | number or string | -
PERCENT_TRADING_TYPE_CHANGE         | Тоже что и `PRICE_TRADING_TYPE_CHANGE` только в процентах.<br>Формат `number` или `low/high` <br>(FAQ п.58)  | number or string | -
