...
**TRADING_PRICE_RANGE_PERCENT**  может иметь значение как одно число, так и диапазон в формате **нижняя граница/верхняя граница**. Например `TRADING_PRICE_RANGE_PERCENT=10` или `TRADING_PRICE_RANGE_PERCENT=10/30`
...
* 2 - Ордера начинают выставлятся от медианы спреда, а уровень безубытка не контролируется. Авто-режим Extra отключен.
...
### 58. Параметры PRICE_TRADING_TYPE_CHANGE и PERCENT_TRADING_TYPE_CHANGE

Только для стратегий **One Orders**.

Параметр переключает тип торговли с All на Buy или Sell при пересечении указанной цены.

1. **PRICE_TRADING_TYPE_CHANGE** может принимать абсолютное значение или нижнюю/верхнюю границу.

    Если задано абсолютное значение то:
...
2. **PERCENT_TRADING_TYPE_CHANGE** может принимать одиночное значение или нижний/верхний уровень в процентах.
    ...
    Полученные границы работают так же как `PRICE_TRADING_TYPE_CHANGE=x/x`
    
При выходе цены за указанную границу в консоли будет уведомление `LOW LINE PRICE` или `HIGH LINE PRICE`.

**Важно!**
* Выбирается один из параметров. Оба параметра не могут работать одновременно.
