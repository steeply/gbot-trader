...
2. Как только цена **LastPrice превысила цену виртуального ордера** (Profit price), **активируется** вилка цен (Max Price и Level SL) для работы StopLoss. 
3. Если цена идет дальше вверх, максимум цены (Max Price) обновляется при этом расчитывается новый Level SL.
4. Как только цена развернулась и пошла вниз, дойдя до Level SL (который является уровнем задаваемым `STOP_LOSS_PERCENT`), StopLoss дает команду и выставляется физический ордер. 
...
6. **Внимание!** Если по какой-то причине StopLoss дал команду на установку ордера, а профитный ордер не установился, то в консоле будет уведомление об ошибке **Problem with StopLoss active**.
...
Level SL:     6582.481         - Уровень срабатывания StopLoss.
``` 

**Важно!** 

* Все расчеты вилки цен и уровня StopLoss происходят в момент обновления автонастроек. 
* При использовании StopLoss "авто-выход из пары" не работает. 
...
Когда установлен фикс ордер и в сетке ордеров не осталось ни одного ордера, при этом LastPrice отдалилась от последнего ордера больше чем на 5 процентов (параметр `OFFSET_LAST_ORDER_PERCENT`) будет автоматически переключена стратегия на обратную.
