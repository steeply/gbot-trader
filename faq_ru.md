...
2. Как только цена LastPrice превысила цену виртуального ордера (Profit price), активируется вилка цен (Max Price и Level StopLoss) для работы StopLoss. 
3. Если цена идет дальше вверх, максимум цены (Max Price) обновляется при этом расчитывается новый Level StopLoss.
4. Как только цена развернулась и пошла вниз, дойдя до Level StopLoss (который является уровнем задаваемым `STOP_LOSS_PERCENT`), StopLoss дает команду и выставляется физический ордер. 
...
LastPrice: 6598.23             - Цена рынка.
Max Price: 6621.5
Level StopLoss: 6582.481       - Уровень срабатывания StopLoss.
``` 
