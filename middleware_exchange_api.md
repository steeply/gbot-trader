...
Название торговых пар должны быть в lowercase.
...
При ошибке, возвращается объект с ключом `error` в котором должно содержатся описание ошибки.
```
{
  success: 0
...
Возвращает историю сделок. Сортировка по возрастанию даты, т.е последние сделки снизу.
...
Биржа BTC-e использует **прямое** название торговых пар, т.е базовая валюта стоит **первой**, например btc_usd.
...
- минимальный объем ордера при обратных парах должен быть эквивалентен минимальному объему btc указаному на бирже (0.0001). Т.е, если у вас валюта Coin стоит 0.000041 btc, минимальный объем ордера в этой валюте будет 0.0001 / 0.000041 = 2.43 
