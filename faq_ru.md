...
Для биржи Poloniex загружается 100 строк из истории за последние 48 часов.
...
5. Если ордер не был исполнен, и цена снова развернулась и пошла вниз, то после исполнения очередного buy ордера Stop Loss ордер будет пересчитан начиная с пункта 1. При этом физический Sell ордер останется на месте до момента, когда должен будет поставиться пересчитанный физический ордер. 
