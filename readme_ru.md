...
> Чтобы установить первый ордер по рынку, используйте `OFFSET_FIRST_ORDERS_PERCENT=-1`
...
/martin [cache] - Теоретический расчет ордеров Мартингейла (параметры берутся из конфига). cache - необязательный параметр, указывающий включить в расчет сетки данные ордеров из кэша истории
