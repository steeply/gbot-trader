...
Когда в сетке ордеров не осталось ни одного ордера, при этом LastPrice отдалилась от последнего ордера больше чем на указанный процентов (параметр `OFFSET_LAST_ORDER_PERCENT`) будет автоматически переключена стратегия на обратную (Если была One Buy, станет One Sell).
...
Параметр позволяет зафиксировать убыток если курс не достиг желаемого уровня.

Когда в сетке ордеров не осталось ни одного ордера, при этом LastPrice отдалилась от последнего ордера больше чем на указанный процентов (параметр `STOP_LOSS_PERCENT`) будет активирована функция Stop Loss.
