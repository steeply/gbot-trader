...
Для использования этой стратегии необходимо задать следующие параметры, на примере бирже Wex и торговой паре ltc/usd: <br />
```
TELEGRAM_OFF=true
KEY=ваш ключ
SECRET=ваш секрет
...
В режиме **_hard_** ордер на продажу поставляется только в том случае, когда объем купленных позиций будет в точности совпадать с кэшем бота. Это означает что все прошло гладко и ни 1 ордер не потерялся и не изменил свой объем.

В режиме **_soft_** (по умолчанию) ордер на продажу ставится в любом случае, на основе данных которые возвращает биржа.
...
Для биржи Wex комиссия берется с сервера биржи, по этому в ручном изменении нет необходимости.
...
Иногда в консоле могу появляться сообщения об ошибках, например:
...
**DANGER_PRICE_STOP** - параметр поставит бота на паузу если за короткий период цена изменилась больше чем на `DANGER_PRICE_STOP_PERCENT` (по умолчанию 9%), например резкий сквиз. Так же пользователю будет выслано уведомление в Telegram. Будут закрыты Buy ордера если скачек был вверх.
